...
import exp_statistics.experiment_2.nonparametric as npc


root = "exp_statistics/experiment_2"
...
if __name__ == '__main__':
    ...
    for i, folder in enumerate(os.listdir(results_dir), start=1):
        ...
        # due to an interface mistake 1 participents were able to finish without 
    ...
    # needed to calculate the Rank-biserial correlation with the R file
    records_per_arm = records.groupby(["participant", "arm", "experience_level"])["jaccard"].mean().reset_index()
    records_per_arm.to_csv("exp_statistics/experiment_2/records_per_arm.csv")
