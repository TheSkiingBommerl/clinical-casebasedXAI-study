...
from AI_diagnostic_support.ecg_embeddings.create_mat import save_signals
from AI_diagnostic_support.ecg_embeddings.embeddings import create_embeddings
from AI_diagnostic_support.similarity_measure.get_test_split import get_test_split
from AI_diagnostic_support.similarity_measure.DTW import process_all
from AI_diagnostic_support.similarity_measure.cosine_similarity import find_similar_embeddings
from AI_diagnostic_support.similarity_measure.create_parquets import build_save_table
from AI_diagnostic_support.automatic_ecg_diagnosis.create_embeddings import create_CODEtest_emb
...
def run_embeddings(data_dir: Path, device: str, db_batch_size: int):
    ...
    for mod_path in [ecg_fm_path, ecg_auto_path]:
        if not mod_path.exists:
            logger.error(f"Model checkpoint {mod_path} Doesn't exist. Did you run AI-diagnostic-support-init-db?")

    signals_path = data_dir / "processed" / "ecg-mat"
    if not signals_path.parent.exists:
        logger.error(f"{signals_path.parent} Doesn't exist. Did you run AI-diagnostic-support-init-db?")
    signals_path.mkdir(exist_ok=True)

    logger.info("Splitting code-test ECG's")
    df_ecg_split = get_test_split()
    df_ecg_split.to_csv(data_dir / "processed" / "ids_similarity_comparison.csv", index=False)
    ...
    create_CODEtest_emb(hdf5_path, ecg_auto_path)

    ecg_auto_dir = simil_dir / "A_ECG_D"
