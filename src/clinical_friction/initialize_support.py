...
from clinical_friction.automatic_ecg_diagnosis.predict import out_pop_prediction
...
def main():
    ...
    filtered_csv_path = processed_dir / "filtered_ecg_experiment_2.csv"
    for path in [code_test_signals, ptbxl_signals, model_path, filtered_csv_path]:
        if not os.path.exists(path):
            logger.error(
                f"{path} not found. Did you run clinical-friction-init-db"
                "and clinical-friction-init-similarity?"
            )
