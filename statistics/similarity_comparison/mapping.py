...
GPT-5.3 was used for assistance
"""

import pandas as pd
import ast
from src.AI_diagnostic_support.database.useful_queries import get_diagnostics

def map(ranking, only_abn=False, only_norm=False):
    mapping = pd.read_csv("statistics/similarity_comparison/data/mapping.csv")
    shuffled = pd.read_csv("statistics/similarity_comparison/data/data_shuffled.csv")  # load shuffled signal IDs
    ...
    result = result.reindex(columns=["id", "DTW", "A_ECG_D", "ECG_FM"])
    final_array = result[["DTW", "A_ECG_D", "ECG_FM"]].values.tolist()
    return final_array